...
class AccessAuthorizer(userDAO: UserDAO, organizationDAO: OrganizationDAO, courseDAO : CourseDAO, quizDAO: QuizDAO, questionDAO: QuestionDAO) extends ProfileAuthorizer[CommonProfile] {
  ...
  def authorized(userId: UserId, accessibleId: AccessibleId, requiredLevel : Access) : Future[Boolean] = accessibleId match {
    ...
    case id : QuestionId => questionDAO.access(userId, id).map( userLevel => userLevel >= requiredLevel)
